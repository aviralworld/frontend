...
const { argv } = yargs
  ...
  .option("keep-originals", {
    default: false,
    description: "whether to leave the original (un-hashed) files in place",
    type: "boolean",
  })
  .option("html", {
      ...
      rewriter.on("startTag", (startTag) => {
        if (startTag.tagName === "link") {
          ...
          if (rel?.value === "stylesheet") {
            ...
            const [, slash, stem, , extension] = hrefAttribute.value.match(
              FILENAME_RE,
            );
